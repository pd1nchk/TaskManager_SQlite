...
namespace AdministradorDeTareas.Model.DAO
{
    public class UsersModelDAO : ITaskManagerServiceDAO<UsersModel> 
    {
        ...
        public async Task<bool> ChangeUserDataName(string newName, string newFullName, string newEmail)
        {
            using (var db = new TaskManagerDbContext())
            {
                UsersModel? validUser = await db.Users.FirstOrDefaultAsync(u => u.UserId == currentUser.UserId);
                if (validUser != null)
                {
                    validUser.FullName = newName;
                    validUser.Email = newEmail;
                    validUser.UserName = newName;
                }
                return db.SaveChanges() > 0;
            }
        }
